...
var diffLine = focus.append('line').attr('class','focus-diff'); // 두 값 사이 세로선

var overlay = uiLayer.append('rect')
  .attr('width', width).attr('height', height)
  .style('fill','none').style('pointer-events','all');

/* 도형 생성기 – series별 y값 계산 */
var maxWorld = 1, maxWilds = 1; // 정규화용
...
function makeY(maxPlayers){
  ...
  if (scaleMode === 'symlog'){
    // 저부 확대: 상수 k가 클수록 0~k 구간이 넓게 펼쳐짐
    return d3.scaleSymlog().constant(5000).domain([0, maxPlayers]).range([height,0]).clamp(true);
  }
  // relative: 0~100%
  return d3.scaleLinear().domain([0,100]).range([height,0]).nice();
}
function y0Base(){
  return scaleMode === 'log' ? y(minY) : y(0);
}
...
Promise.all([ d3.csv(wildsPath), d3.csv(worldPath) ]).then(function(res){
  ...
  overlay
    ...
    .on('mousemove', function(event){
      ...
      if (pw && pz){
        diffLine.style('opacity',1).attr('x1',0).attr('x2',0).attr('y1', yW).attr('y2', yZ);
      } else {
        diffLine.style('opacity',0);
      }
